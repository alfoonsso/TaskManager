# api/app/routers/projects.py — versión completa con recursos anidados
...
def actualizar(proyecto_id: int, datos: ProyectoCreate,
               db: Session = Depends(get_db)):
    proyecto = db.query(Proyecto).filter_by(id=proyecto_id).first()
    if not proyecto:
...
def actualizar_parcial(proyecto_id: int, datos: ProyectoUpdate,
                       db: Session = Depends(get_db)):
    proyecto = db.query(Proyecto).filter_by(id=proyecto_id).first()
    if not proyecto:
        raise HTTPException(status_code=404, detail='Proyecto no encontrado')
